...
else{$rs = mysqli_query($con, "SELECT `address_id`, `address`, `address2`, `district`, city.city as city_id, `postal_code`, `phone`,country.country FROM `address` left join city on address.city_id = city.city_id left join country on country.country_id = country.country_id ");
    //print_r($rs);
    $array = mysqli_fetch_all($rs, MYSQLI_ASSOC);
    //print_r($array[0]);
    $index = 1;
